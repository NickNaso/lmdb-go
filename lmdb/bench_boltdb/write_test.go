...
func openEnv(dbPath string) *bolt.DB {
...
func BenchmarkPutSmall_BoltDB(b *testing.B) {
	b.SetParallelism(1)
	...
	benchPut_BoltDB(b, 20*GB, SMALL_VAL)
}

func BenchmarkPutLarge_BoltDB(b *testing.B) {
	b.SetParallelism(1)
	...
	benchPut_BoltDB(b, 20*GB, LARGE_VAL)
}

func benchPut_BoltDB(b *testing.B, size int64, val []byte) {
	db := openEnv(BENCH_DB)
	...
	checkErr(err)
	bucket, err := txn.CreateBucketIfNotExists(BENCH_DBI)
	checkErr(err)
...
func list(db *bolt.DB) {
	...
	stats := txn.Bucket(BENCH_DBI).Stats()
	checkErr(err)
